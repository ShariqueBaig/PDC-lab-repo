...
# Auto-detect tools if not in PATH
if (-not (Get-Command "cmake" -ErrorAction SilentlyContinue)) {
    Write-Host "CMake not found in PATH. Searching common locations..." -ForegroundColor Cyan
    $CommonPaths = @(
    ...
    foreach ($Path in $CommonPaths) {
        if (Test-Path "$Path\cmake.exe") {
            Write-Host "Found CMake at $Path" -ForegroundColor Gray
...
# Ensure MinGW is in PATH for compilation
